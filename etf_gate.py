...
def evaluate_etf_gate(
    ...
    v8.7: source preference is by file mtime (newest parseable wins):
        1. results/honest_evaluation.md (regenerated by enhanced_backtest)
        2. reports/benchmark_*.md
    Returns a result with severity == "unknown" when neither source yields a
    parseable number (silent: better than a false alarm)."""
    base = Path(base_dir)

    # ---- 1) 取最新可解析的超额收益 ----
    # v8.7 修复：不再无条件优先 honest_evaluation.md——它可能几个月没更新，
    # 而 reports/benchmark_*.md 更新鲜。统一按文件 mtime 从新到旧尝试解析。
    candidates = []
    primary = base / "results" / "honest_evaluation.md"
    if primary.exists():
        candidates.append(primary)
    candidates += [Path(f) for f in sorted(glob.glob(str(base / "reports" / "benchmark_*.md")), reverse=True)]
    candidates = sorted({str(c): c for c in candidates}.values(),
                        key=lambda c: c.stat().st_mtime if c.exists() else 0, reverse=True)
    excess = None
    source = ""
    for cand in candidates:
        v = _read_excess_from_file(cand)
        if v is not None:
            excess = v
            source = str(cand)
            break
